(function(){
	var canvasBody = document.getElementById("canvas_bg"),
			canvas = canvasBody.getContext("2d"),
			
			w = canvasBody.width = window.innerWidth - 100, //Full width
			...
			communicatePoints = function(point1, father){ 
				for(var i = 0; i < father.length; i++){
					...
					if (opacity > 0){ //Draws the line
						...
						canvas.strokeStyle = "rgba(27,166,155,opacity)".replace("opacity", opacity / 2);
						canvas.beginPath();
	...
	window.addEventListener("resize", function(){
		w = canvasBody.width = window.innerWidth-100;
